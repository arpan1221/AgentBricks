...
def kafka_producer_mock() -> AsyncMock:
    """Mock Kafka producer with send_event method."""
    mock = AsyncMock()
    mock.send_event = AsyncMock(return_value=None)  # API uses send_event method
...
async def mock_kafka_producer_dependency(
    kafka_producer_mock: AsyncMock,
    test_app: FastAPI,
) -> AsyncGenerator[None, None]:
    """Override Kafka producer dependency for all tests (autouse=True)."""
...
def mock_aiokafka_producer(monkeypatch):
    ...
    monkeypatch.setattr("src.kafka_producer.AIOKafkaProducer", mock_producer_class)
